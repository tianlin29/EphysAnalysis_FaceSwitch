...
for n = 1:length(file_path)
    ...
    reward = abs(fnd.getp('morph_level'))>0.2; reward = reward(1,:)'; % actually it is easy vs. difficult
...
save(fullfile(InterimDir, 'neu_fnd.mat'), 'neu_fnd');

%% set options
neu_fnd = load(fullfile(InterimDir, 'neu_fnd.mat')).neu_fnd;
...
% set sample number
    % for each dichotomy, pick n_samples=15 trials in each condition for
    % classification. Repeat the process for n_resample=1000 times to get
    % the mean decoding accuracy.  
n_resample = 1; 
...
n_perm_inner = 1; 
    ...
    % pick n_samples=15 trials in each condition for each brain area; 
    % Inf ..use all the trials in a condition 
n_samples = [Inf]; 
...
for idx_brain = 1:length(cell_groups) % for every brain area
    for idx_session = 1:n_sessions % for every session
        ...
        for i_rs = 1:n_resample
            ...
            % decoding accuracy
            [perf,perf_boot,~,perf_boot_raw] = sd(avg_array,n_perm_inner,n_samples(idx_brain));
            ...
            [ccgp_{idx_brain,idx_session}]  = cat(2,ccgp_{idx_brain,idx_session},...
                ccgp(avg_array, n_perm_inner,false,n_samples(idx_brain),'define_sets_cond4'));
        end
    end
...
for idx_brain = 1:length(cell_groups)% for every brain area
    for idx_session = 1:n_sessions % for every session
        ...
        for i_rs = 1:n_resample
            ...
            % CCGP
            ccgp_boot_tmp = ccgp(avg_array,n_perm_inner_CCGP_null,true,n_samples(idx_brain),'define_sets_cond4');
...
%% save results
save(fullfile(InterimDir, 'stat_CCGP.mat'), 'sd_', 'ccgp_');
save(fullfile(InterimDir, 'stat_CCGP_null.mat'), 'sd_boot', 'ccgp_boot');

%% load results
load(fullfile(InterimDir, 'stat_CCGP.mat'));
load(fullfile(InterimDir, 'stat_CCGP_null.mat'));
...
% print(gcf, '-dpdf', fullfile(FigDir, sprintf('%s_%s_decoding_accuracy.pdf', experiment, monkey)));
...
ylim([0 1])
set(gcf, 'Position', [0.12 0.11 0.3 0.25])
% print(gcf, '-dpdf', fullfile(FigDir, sprintf('%s_%s_CCGP.pdf', experiment, monkey)));
