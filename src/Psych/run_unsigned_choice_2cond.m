function [fh, fh_idv, fh_summary, stat] = run_unsigned_choice_2cond(cond, coh, cor, subj, opt)

% select data
def.subj_list = {};
def.subj_idx = []; % will be defined later
% process data
def.log = true;
def.constant = false;
def.verbose = true;
% plot
if length(opt.subj_list)<=10; def.tile_fh_idv = true; else; def.tile_fh_idv = false; end
...
% logit(P) = a0 + a1*cond + a2*coh + a3*coh*cond
% no constant term:
...
% plot
if opt.tile_fh_idv
    fh_idv = figure('color', 'w', 'position', [50+120*(opt.subj_idx-1) 100 120 120]);
else
...
format_panel(gca, ...
    ...
    'xlim', opt.xlim, 'xtick', opt.xtick, 'xticklabel', opt.xticklabel, ...
    'xlabel', {'Stimulus strength','(% Morph)'}, 'ylabel', 'P(correct)');
if ~isempty(opt.subj_list); title(opt.subj_list{opt.subj_idx}); end
