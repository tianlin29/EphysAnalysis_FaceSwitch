function fh = show_flow_field_w_LDS(data, lds_data, opt)
% function fh = show_flow_field_w_LDS(data, lds_data, opt)
% Tstamp = 1 x time
...
pU(Cnt(:) < opt.min_sample) = 0;
pV(Cnt(:) < opt.min_sample) = 0;
...
pU = pU(rowMin:rowMax, colMin:colMax);
pV = pV(rowMin:rowMax, colMin:colMax);

% Plot
fh = figure;
hold on;
quiver(Xgrid, Ygrid, U, V, 'AutoScale', 'on', 'LineWidth', 1.5, 'color', 'k');
off = mean(diff(Xgrid(1,:))) * .2;
quiver(Xgrid + off, Ygrid, pU, pV, 'AutoScale', 'on', 'LineWidth', 1, 'color', 'r');
