function fh = show_flow_field_slice(data, opt)
% function fh = show_flow_field_slice(data, opt)
% Tstamp = 1 x time
...
def.xlabel = 'X';
def.ylabel = 'Y';
def.n_slices_to_show = 5;  % number of slices to show (auto-select with equal intervals)
...
Z = data.Z;
Cnt_all = data.Cnt;
Xgrid_orig = data.Xgrid;
Ygrid_orig = data.Ygrid;
...
nslice_total = length(Z);

% Determine which slices to show
if nslice_total > opt.n_slices_to_show
    % Select evenly spaced slices
    slice_indices = round(linspace(1, nslice_total, opt.n_slices_to_show));
    fprintf('Auto-selecting %d slices from %d total slices\n', length(slice_indices), nslice_total);
else
    % Show all slices if there are fewer than requested
    slice_indices = 1:nslice_total;
    fprintf('Showing all %d slices (requested %d)\n', nslice_total, opt.n_slices_to_show);
end

nslice = length(slice_indices);
ncond = size(mean_trajectory, 2);

fh = figure;

for i = 1:nslice
    sl = slice_indices(i);  % actual slice index
    U = Uall(:, :, sl);
    V = Vall(:, :, sl);
    Cnt = Cnt_all(:, :, sl);
    ...
    Xgrid = Xgrid_orig(rowMin:rowMax, colMin:colMax);
    Ygrid = Ygrid_orig(rowMin:rowMax, colMin:colMax);
    U = U(rowMin:rowMax, colMin:colMax);
    V = V(rowMin:rowMax, colMin:colMax);

    % Plot
    subplot(2, ceil(nslice/2), i);
    hold on;
    ...
    if i==nslice && ~isempty(opt.legend)
    ...
    % Create title with window information if available
    if isfield(data, 'window_info')
        title(sprintf('Window %d/%d (Z = %1.1f)', sl, nslice_total, Z(sl)));
    else
        title(sprintf('Z = %1.1f (%d)', Z(sl), sl));
    end
